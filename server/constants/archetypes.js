// server/constants/archetypes.js
// Source of truth for all 8 archetypes — DO NOT store definitions in DB
...
/**
 * Compute which archetype best matches a given dimension score object.
 * Uses midpoint proximity scoring across all 5 dimensions.
 * @param {Object} dimensions - { dharmaClarity, emotionalDepth, shadowAwareness, actionOrientation, relationshipPattern }
 * @returns {{ archetypeKey: String, matchScore: Number, allScores: Object }}
 */
export function computeArchetypeMatch(dimensions) {
  const dimKeys = ['dharmaClarity', 'emotionalDepth', 'shadowAwareness', 'actionOrientation', 'relationshipPattern'];
  const scores = {};

  for (const [archetypeKey, archetype] of Object.entries(ARCHETYPES)) {
    let totalFit = 0;
    for (const dim of dimKeys) {
      const [min, max] = archetype.dimensionProfile[dim];
      const midpoint = (min + max) / 2;
      const userScore = dimensions[dim] || 50;
      // Normalized fit: 0-20 per dimension (max 100 total)
      const distance = Math.abs(userScore - midpoint);
      const fit = Math.max(0, 20 - (distance / 50) * 20);
      totalFit += fit;
    }
    scores[archetypeKey] = Math.round(totalFit);
  }

  const bestKey = Object.entries(scores).sort((a, b) => b[1] - a[1])[0][0];
  return {
    archetypeKey: bestKey,
    matchScore: scores[bestKey],
    allScores: scores
  };
}
